...
class Services {
    ...
    async getMenuAll() {
        ...
        const response = await sequelize.query(`SELECT * FROM chatbot_hr_menu`,
            {
                raw: true
            }).then(myTableRows => {
    ...
    async getQuestionByText(text) {
        ...
        const response = await sequelize.query(`select question from digitals_shera.chatbot_hr_menu where question like '%${text}%'`,
            {
                raw: true
            }).then(myTableRows => {
                return myTableRows[0];
    ...
    async getRandomQuestion() {
        ...
        const response = await sequelize.query(`select question from digitals_shera.chatbot_hr_menu order by rand() limit 3`,
            {
                raw: true
            }).then(myTableRows => {
                return myTableRows[0];
